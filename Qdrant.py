...
async def main():
    ...
    db = await Qdrant.afrom_documents(split_documents, embeddings, url="http://localhost:6333")

    # 質問
    question = input("質問を入力してください: ")

    docs = await db.similarity_search(question)

    # GPTモデルに質問
    answer = await ask_question(question, docs[0].page_content)
    print(answer)
